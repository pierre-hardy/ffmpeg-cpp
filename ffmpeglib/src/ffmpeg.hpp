...
#define avio_write(a, b, c)                     avio_write(a, (const unsigned char *)(b), c)
#define av_get_sample_fmt_name(a)				av_get_sample_fmt_name((enum AVSampleFormat)(a))
#define av_get_pix_fmt_name(a)					av_get_pix_fmt_name((enum AVPixelFormat)(a))
#define av_dict_set(a, b, c, d) 				av_dict_set(a, (const char *)(b), (const char *)(c), d)
#define av_strlcpy(a, b, c)						av_strlcpy((char *)(a), b, c)
#define av_strlcat(a, b, c)						av_strlcat((char *)(a), b, c)


extern void show_usage(void);
extern int ffmpeg_parse_options(void *p, int argc, char **argv);
extern const OptionDef *get_options(void *p);

class ffmpeg_opt;
extern ffmpeg_opt *alloc_ffmpeg_opt();
extern void free_ffmpeg_opt(ffmpeg_opt *p);
extern void set_opts_ffmpeg_opt(ffmpeg_opt *p,	ffmpeg *ffmp, cmdutils *cmdutils_p, ffmpeg_data *ffmpeg_data_p, ffmpeg_filter *filter);
...
public:
	ffmpeg()
	{
		...
		ffmpeg_data_p = new ffmpeg_data();
		ffmpeg_filter_p = new ffmpeg_filter();
		cmdutils_p = new cmdutils();
		ffmpeg_opt_p = alloc_ffmpeg_opt();

		set_opts_ffmpeg_opt(ffmpeg_opt_p, this, cmdutils_p, ffmpeg_data_p, ffmpeg_filter_p);
		ffmpeg_filter_p->ffmpeg_data_p = ffmpeg_data_p;
		ffmpeg_filter_p->cmdutils_p = cmdutils_p;
		cmdutils_p->ffmpeg_opt_p = ffmpeg_opt_p;
		cmdutils_p->ffmpeg_p = this;
		cmdutils_p->cmdutils_p = cmdutils_p;
	}
	~ffmpeg()
	{
		free_ffmpeg_opt(ffmpeg_opt_p);
		delete cmdutils_p;
		delete ffmpeg_filter_p;
		delete ffmpeg_data_p;
	}
	ffmpeg_data * ffmpeg_data_p;
	ffmpeg_opt * ffmpeg_opt_p;
	ffmpeg_filter *ffmpeg_filter_p;
	cmdutils *cmdutils_p;

	int ffmpeg_parse_options(int argc, char **argv)
	{
		return ::ffmpeg_parse_options(ffmpeg_opt_p, argc, argv);
	}
	...
	const OptionDef  *getoptions()
	{
		return ::get_options(ffmpeg_opt_p);

	}
#define options getoptions()
#define hwaccels ffmpeg_data_p->hwaccels

#define exit_program cmdutils_p->exit_program
#define show_banner cmdutils_p->show_banner
#define uninit_opts cmdutils_p->uninit_opts
#define cmdutils_read_file cmdutils_p->cmdutils_read_file
#define parse_time_or_die cmdutils_p->parse_time_or_die
#define register_exit cmdutils_p->register_exit
#define print_error   cmdutils_p->print_error
#define parse_loglevel cmdutils_p->parse_loglevel

#define init_simple_filtergraph ffmpeg_filter_p->init_simple_filtergraph
#define configure_filtergraph ffmpeg_filter_p->configure_filtergraph
#define ist_in_filtergraph ffmpeg_filter_p->ist_in_filtergraph
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    if (!is_last_report) {
        ...
        if ((cur_time - report_last_time) < 500000)
            return;
        report_last_time = cur_time;
    }
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (resample_changed) {
        ...
        ist->resample_sample_fmt     = (enum AVSampleFormat)decoded_frame->format;
        ist->resample_sample_rate    = decoded_frame->sample_rate;
        ist->resample_channel_layout = decoded_frame->channel_layout;
        ist->resample_channels       = avctx->channels;
...
static int init_input_stream(int ist_index, char *error, int error_len)
{
    ...
    if (ist->decoding_needed) {
        ...
        ist->dec_ctx->get_format            = get_format_static;
...
//                    ost->frame_rate = ist->st->avg_frame_rate.num ? ist->st->avg_frame_rate : (AVRational){25, 1};
            ...
            switch (enc_ctx->codec_type) {
            case AVMEDIA_TYPE_AUDIO:
                enc_ctx->sample_fmt     = (enum AVSampleFormat)ost->filter->filter->inputs[0]->format;
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (!strncmp(ost->enc->name, "mpeg2video", 10) &&
                    ...
                    av_log(NULL, AV_LOG_WARNING,
                           ...
                           "Use -pix_fmt yuv420p for compatibility with outdated media players.\n",
                           av_get_pix_fmt_name(ost->filter->filter->inputs[0]->format));
                enc_ctx->pix_fmt = (enum AVPixelFormat)ost->filter->filter->inputs[0]->format;
...
int ffmpeg_main(int argc, char **argv)
{
    ...
    register_exit(ffmpeg_cleanup_static);
